...
int main() {
	...
	is_running = false;
	std::thread t1(detector_thread, net); // ?´ë??ì„œ while ?±ì„ ?£ì–´??loop.
...
=======
				std::this_thread::sleep_until(starting_point + kMinimumIntervalMs); //ë³‘ë ¬?¤í–‰???„í•´ ? ì‹œ ë©ˆì·„?¤ê°
>>>>>>> 32d31a356676db87fe7c891e75539ac67f5a5bda
			}
...
void detector_thread(cv::dnn::Net &net) {
	...
	while (!is_thread_terminated) {
		...
		cv::Mat inpBlob = cv::dnn::blobFromImage(clone_frame, 1.0 / 255, cv::Size(frameWidth, frameHeight), cv::Scalar(0, 0, 0), false, false); //?´ë?ì§€?„ì²˜ë¦?1/255 ?•ê·œ??
		...
		//std::cout << "?œê°„(ì´? : " << sec.count() << " seconds" << std::endl; // release 0.16 debug 0.19
...
=======
		cv::Mat probMap(H, W, CV_32F, result.ptr(0, n)); //në²ˆì§¸ keypoint??heatmap
		//std::cout << *result.ptr(0, n) << std::endl;
		minMaxLoc(probMap, 0, &prob, 0, &maxLoc); //heatmap ìµœë?ê°??„ì¹˜
>>>>>>> 32d31a356676db87fe7c891e75539ac67f5a5bda
...
#define TWO_STEP_MOVEMENT_SPEED //ì²œì²œ???€ì§ì´?¤ê? ?´ë¦­
